...
Status: code-path audit and read-only database inspection completed. A bounded internal research pilot, automatic completed-day reviews, backtest-isolation fix, reporting qualifications and Mem0 read diagnostics are implemented in this release. The pilot creates restricted rule combinations, tests and refines them, retains a durable SQLite journal and exports versioned JSON artifacts. It is directly bound to the existing discovery tick behind an explicit research configuration. A general LLM inventor, distributed worker service, forward-paper promotion controller and internal allocation engine are **not implemented by this change**. An owner-queued deployment of `cea1650` failed the Windows research-cycle guard and rolled back to `4ed24ad7` without restarting. This followup fixes unclosed test SQLite handles and retains failure diagnostics at the gate's output tail; the specific remote failure cause remains inferred until the Windows gate is rerun. No broker trade or transfer was performed. A new engine boot and per-book receipts remain required to establish deployment.
...
## Automatic reviews of completed accounting days

`StrategyDiscoveryAgent.tick` now calls `learning.daily_review.daily_review_for_book` before its historical research cycle. With research enabled in paper mode, it re-reads the previous seven completed UTC accounting days, excluding the current partial day. This is a calendar accounting window, not an exchange-session calendar. Book-filtered queries paginate recorded closed position rows with exact counts and stable ordering. Short server pages are followed until the count is satisfied; failed, changing-count or incomplete reads produce visible failure receipts rather than fabricated zero-profit days. Reads are not a transactionally frozen database snapshot. All selected source columns were verified against the connected database schema.

Reviews retain recorded P&L and positive/negative row counts by strategy. They explicitly leave verified account return, independent round-trip count, fee reconciliation, open-position NAV and full intraday MFE/MAE unknown. Closed slices and bookkeeping records are not relabeled as independent trades. The observer does not combine `trade_outcomes` with position P&L, which could double-count the same realization.

For eligible stock/crypto rows only, a stored sampled peak can support a price-only giveback observation when its timestamp is inside the holding window and its price, quantity and dollar-peak fields agree. Explicit partial, adopted and complex records, known related partials, unsupported assets and missing or inconsistent evidence remain unknown. These observations cannot reconstruct an unseen intraday path or prove an optimal exit; even apparently simple rows lack complete quantity-history provenance.

The internal journal is `agents/local_state/daily_trade_reviews.sqlite3` by default, alongside the configured research journal, with atomic JSON exports under `daily_review_artifacts/<hashed-book>/`. Immutable records identify book, accounting day, schema and source hash. Repeated unchanged reads reuse evidence; changes within the seven-day rolling window retain another version. A transactional current-review pointer tracks the latest observed source, including corrections that restore an earlier immutable version. Corrections older than that window require a future explicit historical backfill. These private runtime files need host backup and are not automatically synchronized to Supabase.

Evidence-linked hints ask for comparisons of predeclared entry/exit variants after negative recorded outcomes, or profit-protection variants after observed giveback. They remain stored hypotheses: this observer does not automatically feed those hints into the current restricted candidate grammar, change stops, raise risk, or promote strategies. Connecting a validated hint to an appropriate replay worker is a subsequent integration gate, requiring covered paths, costs and independent evaluation. Daily reviews run without Mem0, Discord, LLM or new market-data calls; existing database reads still use infrastructure resources.

## Forex readiness

The existing Forex scanner and Twelve Data/Kraken feed do not establish a usable FX execution venue. The broker-only lane is intentionally dormant, and generic paper accounting incorrectly treats non-USD quote-currency amounts as USD. Bid/ask, financing, currency conversion and session handling also need dedicated validation. Forex was not enabled by this release. The bounded implementation and acceptance sequence is documented in [Forex readiness](TREZO_FOREX_READINESS.md): qualified historical research first, then a dedicated practice adapter and walk-forward evidence. More markets do not establish additional income.
...
Local verification completed September 8: `python -m tests.run_all` passed all 72 suites with zero activity-log writes under the harness's network guard; `python -m pytest -q` passed 1,080 tests. The followup adds 12 completed-day review tests, a regression exercising explicit SQLite handle closure before temporary-directory cleanup, and two tests preserving failure details through deployment-log truncation. The daily-review tests cover the actual Discovery binding for two books, repeated runs, late arrivals, restored earlier evidence, UTC boundaries, short pages, incomplete reads, unsupported observations and storage failures. Four scheduling tests verify the first research tick, hourly repetition, enabled checks and invalid-delay handling. The 17 dynamic-capital tests cover strict routing, failed reads, immutable daily balances, subsequent gains/losses, preserved lineage and fixed-scenario compatibility. The real research discovery-path test retained 24 synthetic trials across two books and three daily cycles without an order or ledger mutation. Earlier synthetic CLI runs verified two successive cycles and eight trials. These tests ran locally on Linux, including a simulated Windows handle-closure check; the actual Windows deployment gate remains necessary. They do not verify a deployed process, vendor connectivity, market profitability or the broader gates below.
