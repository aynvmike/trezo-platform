...
export default function NovaUnified() {
  ...
  const [liveBasePrices, setLiveBasePrices] = useState({ XRP: 1.45, ETH: 2260, SOL: 93.81 });
  ...
  const [marketData, setMarketData] = useState({
    SPY: { price: 0, change: 0, changePct: 0 },
    QQQ: { price: 0, change: 0, changePct: 0 },
    VIX: { price: 0, change: 0, changePct: 0 },
  });
  const [stockDataSource, setStockDataSource] = useState("LOADING");
  const [lastStockFetch, setLastStockFetch] = useState(null);
  ...
  // ── YAHOO FINANCE — LIVE MARKET CONDITIONS ─────────────────────────────────
  // Uses corsproxy.io to bypass browser CORS restrictions
  // Fetches SPY, QQQ, ^VIX as market health indicators
  async function fetchMarketData() {
    const symbols = ["SPY", "QQQ", "%5EVIX"];
    ...
    try {
      ...
      const newData = {};
      results.forEach(r => { newData[r.sym] = r; });
      setMarketData(newData);

      // Determine market condition from SPY + QQQ
      const spyPct = newData.SPY?.changePct || 0;
      const qqqPct = newData.QQQ?.changePct || 0;
      const vix = newData.VIX?.price || 20;
      const avgMkt = (spyPct + qqqPct) / 2;

      let condition = "NEUTRAL";
      if (vix > 30) condition = "HIGH VOLATILITY";
      else if (avgMkt >= 0.5) condition = "HOT 🔥";
      else if (avgMkt >= 0) condition = "WARM";
      else if (avgMkt >= -0.5) condition = "COOL";
      else condition = "COLD ❄️";

      setMarketCondition(condition);
      setStockDataSource("LIVE");
      setLastStockFetch(new Date().toLocaleTimeString("en", { hour12: false }));
    } catch (err) {
      setStockDataSource("OFFLINE");
      // Fallback: simulate realistic market data
      setMarketData({
        SPY: { price: 556.20, change: +(Math.random() * 4 - 2).toFixed(2), changePct: +(Math.random() * 0.8 - 0.4).toFixed(2) },
        QQQ: { price: 480.15, change: +(Math.random() * 5 - 2.5).toFixed(2), changePct: +(Math.random() * 1.0 - 0.5).toFixed(2) },
        VIX:  { price: +(18 + Math.random() * 8).toFixed(2), change: 0, changePct: 0 },
      });
    }
  }

  // Fetch stock market data on mount and every 30 seconds
  useEffect(() => {
    fetchMarketData();
    stockFetchRef.current = setInterval(fetchMarketData, 30000);
    return () => clearInterval(stockFetchRef.current);
  }, []);

  // ── COINBASE PUBLIC API — LIVE PRICES ──────────────────────────────────────
  const COINBASE_PAIRS = { XRP: "XRP-USD", ETH: "ETH-USD", SOL: "SOL-USD" };

  async function fetchLivePrices() {
    try {
      const results = await Promise.all(
        Object.entries(COINBASE_PAIRS).map(async ([sym, pair]) => {
          const res = await fetch(
            `https://api.coinbase.com/v2/prices/${pair}/spot`,
            { headers: { "Content-Type": "application/json" } }
          );
          const data = await res.json();
          return [sym, parseFloat(data.data.amount)];
        })
      );
      const newPrices = Object.fromEntries(results);
      setLiveBasePrices(prev => {
        setPriceChange({
          XRP: +((newPrices.XRP - prev.XRP) / prev.XRP * 100).toFixed(3),
          ETH: +((newPrices.ETH - prev.ETH) / prev.ETH * 100).toFixed(3),
          SOL: +((newPrices.SOL - prev.SOL) / prev.SOL * 100).toFixed(3),
        });
        return newPrices;
      });
      setPrices(prev => ({
        XRP: newPrices.XRP,
        ETH: newPrices.ETH,
        SOL: solUnlocked ? newPrices.SOL : prev.SOL,
      }));
      setPriceSource("LIVE");
      setLastFetch(new Date().toLocaleTimeString("en", { hour12: false }));
    } catch (err) {
      setPriceSource("OFFLINE");
    }
  }

  // Fetch live prices on mount and every 15 seconds
  useEffect(() => {
    fetchLivePrices();
    fetchRef.current = setInterval(fetchLivePrices, 15000);
  ...
  useEffect(() => {
    ...
    intRef.current = setInterval(() => {
      ...
      // ── UPDATE CRYPTO PRICES (live base + micro drift for indicators) ───
      setPrices(prev => {
        const next = { ...prev };
        Object.keys(CRYPTO_COINS).forEach(sym => {
          if (sym === "SOL" && !solUnlocked) return;
          // Use live price as anchor, add tiny ±0.1% micro-drift for
          // realistic intrabar movement between 15s API refreshes
          const base = liveBasePrices[sym] || prev[sym];
          const microDrift = (Math.random() - 0.5) * 0.001;
          next[sym] = +(base * (1 + microDrift)).toFixed(base > 100 ? 2 : 4);
        });
        return next;
      });
      ...
      // ── STOCK BOT ──────────────────────────────────────────────────────
      // Adjust aggression based on real market conditions
      const isHotMarket = marketCondition.includes("HOT") || marketCondition === "WARM";
      const scanThreshold = isHotMarket ? 0.40 : 0.55; // scan more in hot markets
      if (stockActive && inWindow && dailyLossStock < STOCK_ACCOUNT * 0.10) {
        const found = Math.random() > scanThreshold;
                ...
                <div style={{ background: T.card, border: `1px solid ${T.green}44`, borderRadius: 10 }}>
                  <div style={{ background: T.green + "14", borderBottom: `1px solid ${T.green}33`, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      ...
                      <Tag color={stockDataSource === "LIVE" ? T.green : stockDataSource === "LOADING" ? T.gold : T.red} sm>
                        {stockDataSource === "LIVE" ? "🟢 LIVE" : stockDataSource === "LOADING" ? "⏳" : "🔴 SIM"}
                      </Tag>
                    </div>
                  </div>
                  <div style={{ padding: "12px 14px", display: "grid", gap: 8 }}>
                    ...
                    <div style={{
                      ...
                      border: `1px solid ${marketCondition.includes("HOT") ? T.green : marketCondition.includes("COLD") ? T.red : T.border}44`,
                    }}>
                      <div style={{ fontSize: 8, color: T.muted, letterSpacing: ".12em", marginBottom: 6 }}>MARKET CONDITIONS — LIVE</div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
                        {Object.entries(marketData).map(([sym, d]) => (
                          <div key={sym} style={{ textAlign: "center" }}>
                            ...
                            <div style={{ fontSize: 12, fontWeight: 700, color: T.text }}>${d.price}</div>
                            <div style={{ fontSize: 9, color: d.changePct >= 0 ? T.green : T.red }}>
                      ...
                      <div style={{ marginTop: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: 9, color: T.muted }}>Cycle:</span>
                        <span style={{ fontSize: 10, fontWeight: 700, color: marketCondition.includes("HOT") ? T.green : marketCondition.includes("COLD") ? T.red : T.gold }}>
                          {marketCondition}
                        </span>
                      </div>
                    </div>
                    {[
                      ...
                      ["Data Source", "Yahoo Finance (SPY/QQQ)", T.green],
                      ["Last Fetch", lastStockFetch || "Fetching...", T.muted],
                    ].map(([k, v, c]) => (
                ...
                <div style={{ background: T.card, border: `1px solid ${T.cyan}44`, borderRadius: 10 }}>
                  <div style={{ background: T.cyan + "14", borderBottom: `1px solid ${T.cyan}33`, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      ...
                      <Tag color={priceSource === "LIVE" ? T.green : priceSource === "LOADING" ? T.gold : T.red} sm>
                        {priceSource === "LIVE" ? "🟢 LIVE DATA" : priceSource === "LOADING" ? "⏳ LOADING" : "🔴 OFFLINE"}
                      </Tag>
                    </div>
                  </div>
                  <div style={{ padding: "12px 14px", display: "grid", gap: 8 }}>
                    {[
                      ...
                      ["XRP", `$${prices.XRP} ${priceChange.XRP >= 0 ? "▲" : "▼"} ${Math.abs(priceChange.XRP).toFixed(3)}%`, priceChange.XRP >= 0 ? T.green : T.red],
                      ["ETH", `$${prices.ETH} ${priceChange.ETH >= 0 ? "▲" : "▼"} ${Math.abs(priceChange.ETH).toFixed(3)}%`, priceChange.ETH >= 0 ? T.green : T.red],
                      ["SOL", solUnlocked ? `$${prices.SOL} ${priceChange.SOL >= 0 ? "▲" : "▼"} ${Math.abs(priceChange.SOL).toFixed(3)}%` : "Locked ⏳", solUnlocked ? (priceChange.SOL >= 0 ? T.green : T.red) : T.gold],
                      ["Last Fetch", lastFetch || "Fetching...", T.muted],
                      ["Data Source", "Coinbase Public API", T.cyan],
                    ].map(([k, v, c]) => (
                      <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 11, padding: "4px 0", borderBottom: `1px solid ${T.border}44` }}>
                        <span style={{ color: T.muted }}>{k}</span>
                        <span style={{ color: c, fontWeight: 600 }}>{v}</span>
