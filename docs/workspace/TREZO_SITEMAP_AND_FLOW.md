...
| `/dashboard/live` → `/paper` · `/performance` → `/dashboard` · `/projections` → `/budget` · `/home` & `/overview-preview` → `/dashboard` · `/trading-preview` → `/paper` · `/agents-preview` → `/agents` | Redirects | legacy + retired duplicate views |
...
- **Overview / Trading / Agents** are single tabs (duplicates removed 2026-06-19): **Overview** is the redesigned Neo Obsidian glance view; **Trading** and **Agents** are the feature-rich workhorse pages that keep the action tools (close position, toggle/run agents). Retired duplicate/preview routes redirect into these.
