...
> ## 2026-08-27 (Nova) — CURRENT STATE: 30 agents on 3 live books, dividend lane + report pipeline + boot-verified deploys, the 52-item audit answered same morning
>
> **Start here.** This entry is a full state-of-the-platform read as of commit `84e43cf`, written the morning the audit clear-out shipped. Everything below it is history.
>
> **DEPLOYMENT.** Server: Lightsail "Trezo-Server" (98.81.100.112), `TrezoAgents`/`TrezoWeb` under nssm. Engine = FastAPI + APScheduler, **30 agents**, ALL silence-watched by ops_watchdog (was 24/30 until d1becae). Deploy path: push to `main` → `python3 ops/relay.py deploy` (Supabase ops relay queues `git_pull_restart`; six CHECK-constrained kinds, never widen) → **the boot beacon proves it**: every boot writes `engine_boot` (pid, short commit, agent count); `relay.py watchboot` confirms. *A deploy is done when a NEW process says hello* — never when the job row says done; on 8/26 the job said done while the engine ran 8-day-old code, and only the beacon ended that class of lie. The detached self-restart was hardened 8/27 (schtasks as SYSTEM/HIGHEST, +2min start, outcome captured, failure alerts `restart_not_scheduled`) but is **unverified live** — the fallback that always works: Mike runs `nssm restart TrezoAgents` in an admin RDP PowerShell. nssm printing `SERVICE_START_PENDING` is NORMAL (Python takes ~40s; watch the beacon, not nssm). The 8/26 restart-loop (3 boots in ~21 min, then settled) is still unexplained — watch for recurrence. **Standing ask: `AUTO-PULL.ps1 -Register` has still not been run on the server.**
>
> **THE BOOKS — all three live.** One owner, three paper books, per-book credentials via the accounts registry: primary `PA3PR4F6ZFWZ` · 25k `PA3YIQ5GH703` (env slot _2, label "Trezo Inc. 3" — labels are crossed, see 8/09) · 75k `PA396OKQTR2H` (slot _3). All three authenticate; agent traffic fans out per book; risk per 8/09's `0049` (4%/4%/10%).
>
> **LANES, honestly.** *Stocks:* 3 books on the market-wide pool (by-trades ranking since 8/10). *Crypto:* the only lane making real money (+$11.6k lifetime) AND the one whose fills are modeled (venue has no paper API) — least externally validated number in the system; since d1becae every such fill records `venue_fallback_modeled` instead of pretending. *Options/wheel:* decision chain works end-to-end; **zero realized track record** (5 adopted positions, none closed; one account still lacks the options-approval dashboard toggle). The wheel advisor is now REAL, not decorative: tier rule (GROWTH never wears a call — advisor + CC-overlay gate), ex-date guard, ledger-honest collateral with **defer-or-SHRINK** (computes the affordable contract count), earnings blackout (cycle-awareness feed), market-pressure deferral, and it judges the ACTUAL submitted OCC. Universe = one rotating bench (`_ordered()`), open positions first. *Forex:* Twelve Data adapter built (7-credits/min budget, 10-min cache, Kraken fallback); lane DORMANT by Mike's explicit choice pending the broker decision. *Dividends (Long-Term):* the newest lane — §1 sizing with hard guardrails, per-name state machine, five lane rules; the screen splits the company test (yield ≥1.5%, payout ≤70%, raise streak, and a cut readmits after recovered+rising+3-healed-years) from the fund test (trailing distribution ≤ trailing total return, no reverse split in 24mo, $2M ADV floor); the data layer is the broker's corporate-actions feed, split-adjusted, with specials/fragments filtered. **Migration 0058 applied 8/27**: `dividend_lane_mode` (ACCUMULATE|INCOME|PARTIAL) + `partial_pct` live per book — the §6 INCOME draw (min(actual distributions, 90% of trailing TR)) is reachable at last; flip per book in bot_settings, no deploy needed. First real pay date ~mid-Sept answers whether Alpaca paper credits dividends (8/27 probe: endpoint works, 0 DIV rows on all three books so far).
>
> **MARKET REPORTS → AGENTS (live since 8/25-26).** Scheduled tasks at 8:00 AM + 3:00 PM ET build the market report and post `market_context` via `ops/relay.py brief` → `relay_briefings` → relay_ingest → **market_desk** builds ONE MarketView (regime/indices/vix/breadth/movers/catalysts). Single reader `current_market_view()`; consumers are TIGHTEN-ONLY: risk_manager +5 effective TCS in risk_off (veto text cites the report), the wheel advisor defers new CSPs on names the report lists under pressure, the dailies read the desk first. Missing/stale view = "no opinion", never a freeze. The watchdog's own 8:30a/3:25p brief windows were freed from the 8-PM janitor gate on 8/27 — they had never once fired.
>
> **THE AUDIT (8/27).** Mike audited `e08649d` → 52 items (9 fail silently, 15 don't bind, 18 half-built, 10 not started; root pattern: **built but not bound**). Same-morning response shipped pre-open as `d1becae`: **22 fixed** (all five priority items: relay alert honesty, reevaluator ex-div exemption, per-name cap, Alpha Vantage burn replaced by corporate-actions yield, paper-dividends probe), 1 probed, 2 partial, **27 deferred with written reasons** — the recompiled `docs/workspace/DEFERRED_ITEMS_TRACKER.md` is now the canonical deferred list. Python suite **321 tests green**; JS/TS tests still zero (deferred to the web pass).
>
> **OPS + SECURITY.** Secrets exist ONLY in local .env files (agents/api/web), never committed. The USB mirror STRIPS `.env*`, purges any the stick already carried, and writes sanitized key-name templates; **one double-click restores a PC**: `RESTORE-FROM-USB.cmd` at the stick root copies everything back, recreates each .env as an empty skeleton, and lists exactly which keys to refill from the password manager (`84e43cf`; REBUILD-FROM-USB.md rewritten to match). Kill-switches: daily 3% / weekly 6% / streak 3 / rejects 3-in-60min rolling / slippage 3×75bps (live via stocks_reconcile fill measurement).
>
> **THE WEEK'S COMMITS** (each boot-verified on the server): `2c8c10d` fund-vs-company screen split · `eb271bb` repaired-cut readmission + split/specials/fragments data layer · `0f4fe5d` forex adapter · `fbb1f61` wheel bench rotation + dailies regime read · `b4ec566` Market Desk + consumers · `e08649d` boot beacon + verify_boot · `d1becae` audit clear-out (16 files, +618/−140) · `84e43cf` USB restore flow. Reference: the 36-page **Trezo Engine Reference** PDF (8/23, at eb271bb) documents agents/gates/formulas/strategies/limitations.
>
> **OPEN / WATCH:** (1) AUTO-PULL register on the server. (2) The next relay deploy is the first live test of the hardened detached restart — expect `restart_scheduled` then a beacon with no human involved; if absent, it's still manual-restart land. (3) Mid-Sept: the dividend-credit verdict, then the two-ledger reconciliation it gates. (4) Restart-loop recurrence. (5) Options approval toggle on the lagging account. (6) Suggested deferred-pile order: web pass with CI → assignment flow → bare-`pass` sweep → dead-symbol deletion → scanner extraction.
...
>
> **THE BOOKS.** Mike added two paper accounts under his ONE Alpaca login. All 3 authenticate live, numbers match the dashboard: `trezo_claudecowork` PA3PR4F6ZFWZ $4,856 (primary) · `Trezo Inc. 3` PA3YIQ5GH703 $25k (env slot **_2**) · `Trezo Inc. 2` PA396OKQTR2H $75k (env slot **_3**). *Note the labels are crossed — Inc.3 is the 25k.* Owner `cf1b0460-039d-40ac-adc8-7ca3ef17c5bb`; books `6ce61054-7ffd-41b5-80c3-1cd0220c79eb` (25k) and `49acafdd-1c86-4740-a1b1-f94aa7abce08` (75k).
>
> **⭐ THE MODEL: owner (person) vs account (book).** Mike caught the flaw — *"people can have multiple live accounts"*. Every table keyed `user_id`, which meant BOTH. `paper_accounts.user_id` FKs to `auth.users` (25 tables do, all CASCADE) so a book id could not be synthetic. **Solution (`0047`): repoint the FKs** — book tables → `trading_accounts(account_key)` → `auth.users`. Person → accounts → books; CASCADE survives transitively; **column keeps its name and ZERO application code changed**, because the engine already treated `user_id` as the account key. Rejected: fake auth users per book (breaks `auth.uid()` RLS for real customers) and adding `account_key` to ~20 tables.
>
> **MIGRATIONS APPLIED (all verified against live schema):** `0045` owner/account split (+`trading_accounts`, `owner_id`/`funding_account_key` on kindrip, `owner_id` on payments) · `0046` seeds paper_accounts + bot_settings for the 2 new books · `0047` repoints 17 FKs, rewrites 19 RLS policies via new `public.my_account_keys()` (STABLE, SECURITY DEFINER, pinned search_path) · `0048` `watchlist_items` (indirect policy the sweep couldn't see) · `0049` per-account risk. Plus 3 `DIAGNOSTIC_*.sql` kept as reference.
>
> **RISK PER BOOK (`0049`):** primary 4%×14 · 25k 4%×14 · **75k 10%×6**. max_open was cut WITH the raise: 10%×14 = **140% of the book at risk** (impossible), and `portfolio_risk` measures 14 positions as **6.83 effective bets, 64% one factor** — correlated stops make the worst case realistic. All three now ~56/56/60% total exposure, so only BET SIZE differs. **⚠️ `optimal_f` over the 196 real closed trades returns 0.001 (the floor) — geometric mean < 1.0 at EVERY fraction (98W/98L, −$386.75).** 4% → −0.073%/trade, 13.9% DD; 10% → −0.190%, 32.4% DD. Raising f multiplies loss, not return. That window spans the scalp-exit defect, the 33× kill-switch and the execution leaks — a partly-broken system, not a verdict. **The 75k book at 10% is the test of whether the repairs changed the edge.**
>
> **⭐⭐ THERE WAS NO LOOP FAN-OUT TO BUILD — DO NOT BUILD ONE.** Trezo was already multi-account at the data layer: `trade_execution` has always had `users = [a['user_id'] for a in paper_accounts]` + `for uid in users: _execute_for_user(...)`; `risk_manager` sweeps `paper_accounts`; `position_monitor` selects `paper_positions` with NO user filter and carries each row's `user_id`. A scheduler fan-out would TRIPLE-PROCESS — and `dividend_manager`/`kindrip` sweep all users too, so fanning them would **triple-pay distributions**. **The one real bug was that nothing bound the CREDENTIALS**, so all three books' orders/exits would have hit the PRIMARY Alpaca account. Fixed at the action points: `bind_for_user()` at trade_execution's 2 call sites; `set_account_for_user()` (inline binder — position_monitor's per-row body is ~500 lines and re-indenting live exit code was the riskier edit) at `for r in rows:`; and **`should_skip_unresolved()`** because an unknown book used to fall back to the PRIMARY account — now skipped, but only when multi-account is active.
>
> **NEW FILES:** `agents/app/brokers/accounts.py` (registry; ContextVar so concurrent per-account work can't leak; DROPS an account on truncated creds / duplicate key id / duplicate book key rather than repairing it) · `agents/app/dividends/schedule.py` (real ex-dates: Finnhub → Alpha Vantage fallback, cached 1 call/symbol/day, fails open).
>
> **TWO LATENT BUGS KILLED.** (1) **Duplicate keys in a .env do NOT create accounts — dotenv keeps only the LAST value per name.** Three pasted `ALPACA_API_KEY`/`SECRET` blocks had crossed account #2's key id with account #3's secret, on a base_url ending `/v2` (code appends `/v2/...` ⇒ 404s). (2) **`TREZO_PRIMARY_USER_ID` sat in agents/.env but was read with `os.getenv`, which never sees that file** — so single-row settings mode NEVER engaged since 7/6 and `get_bot_settings()` silently used "most recently updated row". Now a Settings field. ⚠️ Fixing it then made all 3 books read the PRIMARY's settings, so the anchor was generalised: one account behaves as before, several each read their own row.
>
> **DIVIDENDS.** `drip.py` had NO payout-frequency field — `DISTRIBUTION_INTERVAL_DAYS=7` hardcoded, so **every holding paid weekly**. Annual dollars right, **timing entirely synthetic** ⇒ ex-date laddering (Mike's actual ask) was untestable and a quarterly payer got 52 compounding events/yr instead of 4 (**6.7 pts/yr of phantom compounding at a 60% yield**). Now: per-holding frequency (unknown ⇒ quarterly, the conservative choice), `ex_date_price()` (DRIP buys AFTER the drop), `total_return()`, and **`yield_trap()`** — fires when income>0 and total<0, i.e. the AIYY signature. Verified on Mike's real loss shape: income +40%, price −60%, **total −20%**, trap fires. `dividend_manager` uses the DECLARED $/share when the calendar has one and writes `last_distribution_date` = the ex-date so a missed payout is caught next tick. **CAVEATS: live fetch UNTESTED (parsers verified on real-shaped payloads); no `dist_frequency` column exists yet ⇒ everything defaults quarterly, which UNDERSTATES genuinely weekly YieldMax funds — belongs in the UI next to the DRIP toggle.**
>
> **SUPABASE SQL EDITOR GOTCHAS (cost 3 failed runs).** (a) Two `DO $do$` blocks in one script ⇒ *"unterminated dollar-quoted string"* — the dashboard splitter pairs identical tags across the WHOLE script; **give every block a unique tag**. (b) **`information_schema` HIDES foreign keys into the `auth` schema** (privilege filtering) — a constraint query returned 0 rows while the FK plainly existed; use `pg_catalog`/`pg_constraint` with `confrelid='auth.users'::regclass`. (c) `ADD CONSTRAINT ... FOREIGN KEY` **validates every existing row** — seed the parent from EVERY child table first. (d) Mike re-pastes into an open tab ⇒ **stale paste**; tell him to clear it.
>
> **NOVA CAN REACH Alpaca AND Supabase from the sandbox** (earlier "can't verify live" was wrong) — but **cannot run DDL** (PostgREST does none, no Postgres password in .env). Nova CAN read the live schema via the PostgREST OpenAPI endpoint to verify a migration before and after.
>
...
> **2026-07-20 (Nova) — LANDED commit `31ee984` (5 files, intraday on Mike's "complete all"): Rulebook 5.4+5.5, margin earns +8, wheel weeklies, reprice SHADOW — plus the forex card fixed web-side.**
> ENGINE (loads at next restart): (1) wheel.py — `decay_rate_monthly` + decay-PROJECTED CSP strikes (5.4: an OPTION on names already on the bench, never a preference) + `evaluate_cc_recovery` (5.5 arithmetic gate: no symbol bans; distress = 15%+ under basis → OTM-only strikes, income pace must beat decay pace, premium non-trivial; ledger decides) + `dte` param through all evaluators. (2) options_scanner.py — `_run_cc_overlay` (CCs against ANY 100-share broker lot, 1 write/day, no stacking, recovery-ledger note, rides wheel_cc plumbing) + `_wheel_dte_pick()` targets posture-cap−1 so WEEKLY expirations qualify (ends the "DTE 32 > 21" full-wheel bench between monthlies — today's logs showed the entire bench blocked) + "decay-projected" tag in wheel notes. (3) engine.py — stock entries may spend cash + (X−1)·equity (TREZO_MAX_DEPLOY_X=1.25 closed-form ceiling; self-shrinks to zero at the cap); crypto/forex stay cash-only. (4) risk_manager.py — MARGIN TERRITORY: broker cash < 15% of equity (60s cached snapshot) ⇒ stock entries need +8 TCS (TREZO_MARGIN_TCS_BUMP, stacks with every other bump); approvals tagged `leveraged` for ledger-separable outcomes. (5) reevaluator.py — reprice SHADOW ONLY (Mike: "we just implemented the 8% rule… find a way that makes sense before changing things without data"): live 8% far-trigger untouched; a 3% trigger (TREZO_REEVAL_SHADOW_FAR_PCT) logs `reprice_shadow` rows (would-be target, 6h/position throttle). 30-day evidence: 7 of 9 green-peaked round-trippers sat UNDER 8% (≈ −$191: RBLX −43, SNDQ −90, SNXX −39, SOXS −14…) — Friday's shadow read decides the knob.
> WEB (live on reload, tsc+eslint 0): dashboard/paper card — forex rows get layer-6 "Forex" badge (was "Stock"), `forex_swing` thesis copy (was the crypto-swing template), and LIVE Kraken spot via `fetchForexQuotes` (short-aware P&L/% on the card; was "Live price unavailable" + 0%). DB rows were always correct — display-layer only.
> AM findings (Mon 7/20): forex shorts green (EURUSD +$0.47, GBPUSD +$0.23); crypto idle = DISCIPLINE not defect (ETH TCS 43–47 vs floor 50 on 0.5× volume; ETH banked +$31.42 Thu via the continuous trail); DRAM stale $55.74 target = the 8% blind zone (hence the shadow); NVDA 207.5C 0.30→1.12 happened 9:30–9:34, BEFORE the same-day lane's 9:35 window → NEXT BUILD: agent-driven PRE-BELL HOT LIST (pre-market gappers/volume leaders, pre-computed strikes, early entry from ~9:31 with first-minute confirmation; manual editing only behind the expert-mode toggle). Restart pending (Mike runs it); Rulebook docx chips 5.4/5.5 → LIVE + rule 1.9 added same session.
> **2026-07-16 #4 (Nova) — library index PERSISTED (18ms cold loads) + the quant folder is a true DROP-BOX.**
> (1) `_index.json` beside the texts: `_build()` loads it when its stored folder-signature matches (verified: 18ms cold restart vs full re-parse), rewrites it after every rebuild — no new process, no new agent, exactly the "index in its own home" promise kept cheap. (2) `library.sweep_local_sources()` (shared): mirrors C:\Trezo\Quantconnect + TREZO_PROJECT/06_external_research (.txt/.md/.py/.cs, ≤3MB, mtime-aware) into the library; ops_watchdog runs it DAILY (`library_sweep` activity when files land) — Mike drops files in the quant folder and the agents know them within a day, no script run; PDFs still need one `-m scripts.build_library` pass (pypdf). Library now 16 docs / 1,358 passages. Sandbox py_compile hit the mount's __pycache__ flake — verified via explicit cfile to /tmp instead (note for future: use `python3 -B` + py_compile cfile on this mount).
> **2026-07-16 #3 (Nova) — both option lanes go MARKET-WIDE (Mike: "expand beyond the watchlist... do not limit... scan a few industry winners").**
> Same-day lane candidates: SPY/QQQ + leading-sector generals + the market-wide movers pool (most-actives/gainers/losers, hour-rotating) merged + deduped, cap 24/scan. Wheel universe gains a MARKET-SCAN bench: leading sectors' generals + most-actives join as source='market_wide' WHEN their CSP collateral fits the account (spot ≤ (25%·equity)/100/0.95 ceiling via effective_equity; ≤15 adds/build); no dividend gate — evaluate_csp judges premium on merit; seeds/watchlist/position sources unchanged; daily seed rotation from #2 still applies. Watchlist = personalization, never the universe — now true for options too. Restart loads.
> **2026-07-16 #2 (Nova) — "why is Ford the only put?" answered (collateral arithmetic, not bias) + wheel bench widened/rotated + same-day capacity up (PDT-gone dividend).**
> Diagnosis from the actual universe builder (wheel_universe.get_wheel_universe = curated seed + dividend-friendly watchlist adds; NO legacy-data favoritism): at ~$4.8k equity the growth-posture wheel allowance is 25% ≈ $1,215 and a CSP reserves strike×100 — F @ $12.5 = $1,250 was the ONLY name on the whole seed list that fit (everything else $1.6k–$5k); add the 1-CSP growth cap + FIXED scan order and F wins every cycle by arithmetic. FIXES: (1) Tier D seed adds — AGNC/NOK/VALE/KGC/PSEC (liquid, income-tilted, sub-$13 collateral fits) so the bench is wider than one stock until equity grows into Tiers A–C; (2) seed order ROTATES by calendar day (every affordable name takes a turn at the single slot). (3) SAME-DAY CAPACITY (Mike: "+15% more income... take advantage of the PDT change"): TREZO_DAY_OPT_OPEN 1→2, PER_DAY 2→4, new PER_SYM=2 (re-entry allowed after the first round trip closes; `D:date:SYM#n` counted keys). Quality gates unchanged (move/volume/dust/morning window). Restart loads.
> **2026-07-16 (Nova) — the 300-THRESHOLD mystery solved (Bot Tuning slider min was 300 on the old scale!) + continuous crypto profit trail + PRE-HOLIDAY review.**
> (1) Pattern Detection's "TCS threshold 300, nothing fired" traced: the SQL conversion worked, but the Bot Tuning SLIDER still had min={300} max={1000} — any save clamped tcs_threshold to ≥300 (impossible on 0–100), muting the pattern lane while own-floor scanners kept trading. FIXED three deep: primary row 300→50 (REST, immediate — no restart needed for the threshold); slider re-bounded min 30 / max 95 / step 1 with old-value coercion; engine `_from_row` self-heals any >100 write (÷10). (2) CONTINUOUS PROFIT-LOCK TRAIL for crypto (Mike: "I would prefer a trailing stop… the profit lock is the key" re the ETH 33%-giveback alert): swing rows now take max(ladder rung, stock-style 30%-giveback trail) — between the coarse rungs the stop ratchets continuously, so a pullback books ~70% of the PEAK gain instead of falling to the last rung. Advisor alerts stay advisory BY DESIGN (the acting layer = ladder+trail+giveback tighten); answer to "do I have to liquidate myself" = NO. (3) PRE-HOLIDAY REVIEW (position_monitor, break's eve 2:00–3:45 ET, once): long-term lanes (hodl/dca/dividend/kindrip/wheel) ride; locked-profit positions ride; green-but-unprotected → stop raised to BREAKEVEN + broker resync; RED short-horizon → close_requested before the break ("no time to re-evaluate right after — sell before, unless long-term"); crypto exempt (24/7); `preholiday_review` line per position. Restart loads (2)+(3)."
> **2026-07-15 #7 (Nova) — the QUIET-AFTERNOON diagnosis: leaked open-signal slots (not the goal bump, not the pockets) + PYPL loose ends tied.**
> Mike asked why little happened after the goal was hit. DB truth (mounted log froze at 5:29 AM — stale-mount disease): since the open 0 persisted signals / 0 approves / 19 vetoes, afternoon vetoes ALL "Open-signal cap reached (10)" + one "Already approved AAL" (AAL had NO position = a leaked slot). Root cause: `_recent_approvals` (dedupe set = the cap pool) only shrinks on kind="close" messages or execute info/error — closes via reconcile/monitor paths and silent execute deaths never free their slots; seeding refills from open positions at restart, so 6-8 slots start occupied and a few leaks saturate the cap. FIX: set → dict(ticker → approved-at; 0.0 = position-backed seed) + `_prune_approvals()` every 10 min — a slot survives only if (a) ticker has an OPEN paper_positions row or (b) approval < TREZO_APPROVAL_TTL_H (2h, in-flight); frees the rest with `approval_slots_freed` activity. PYPL wrap: engine had market-sold the last 4 @ 54.25 at 9:40 (row already closed by reconcile; day total ≈ +$93.5); Nova CANCELED the orphan protective stop (52.66 × 4) left at Alpaca — it would have opened a naked short. F 12.5P row confirmed OPEN in ledger (paper page "not yet tracked" card = display matcher quirk, cosmetic — adopt pass + restart make it moot). bot_settings verified clean of 1000-scale values. Restart loads the slot fix + today's edge-guards.
> **2026-07-15 #6 (Nova) — Mike's three edge-guards: open-bell GAP audit, weekend/holiday THETA guard, STALLED-PREMIUM harvest.**
> (1) GAP CHECK (position_monitor `_gap_check_open_bell`, once/day 9:30–10:00 ET, ≤12 held stocks): |gap| ≥ TREZO_GAP_ALERT_PCT (3%) vs yesterday's close → gap-DOWN not through the stop: stop tightened to ~2% under the open + leg-resync to the broker; gap-UP: logged read ("trail ratchets the lock; TP fills at the better price"); every gap gets a `gap_check` audit line — the PYPL lesson generalized. (2) WEEKEND/HOLIDAY THETA GUARD (options re-score): `_multi_day_break()` via Alpaca clock (next_open − today − 1: 2 = weekend, 3+ = holiday); afternoon (≥2 PM ET) before a ≥2-day break → LONG options with DTE ≤ 7 exit in full ("banking before the bleed" — theta burns across closed days with no chance to react); Monday re-entry re-scores fresh via existing hourly reeval + regime reads. (3) STALLED PREMIUM (short branch): `_short_low_at` tracks when the premium low last improved; earned ≥45% (TREZO_OPT_STALL_RATIO 0.55) + DTE ≥ 10 + no progress ≥3 days (TREZO_OPT_STALL_DAYS) → buy back all ("freeing the collateral to work elsewhere") — the F-put-parked-capital case. Restart loads all three.
> **2026-07-15 #5 (Nova) — advisories stop lingering on auto (Mike: "it should go to a system log and be held on the backend for audits... I am not going to stop the agents unless it's really urgent").**
> Web: exit-advisor query = URGENT (any age) OR raised within 45 min (`.or(severity.eq.urgent,raised_at.gte.cutoff)`); header reads "needs your eyes" only when an urgent is present, else "Recent advisories · auto-clears; full history kept for audit". Also #prior: the alert stack now FLOATS as a pinned scrollable card ≥1900px (right rail, 350px, internal scroll) and is inline-capped 38vh below that — the Trading page never grows. Engine: ops_watchdog hourly sweep stamps acknowledged_at on non-urgent alerts older than 2h (rows KEPT = audit trail; `alert_autoclear` activity when >0; `_ALERT_ACK_HOUR` marker; urgent never auto-cleared). Manual/pro-mode full-history view = future pass (table already holds everything).
> **2026-07-15 #4 (Nova) — the PYPL open-bell partial-fill incident: leg-resync hardened to BROKER-TRUTH qty + protection-first fallback.**
> Mike asked why PYPL's TP sold only 10 of 14. Reconstruction: overnight +16.8% gap → at 9:30:54 the profit-trail ratcheted and resync_alpaca_legs canceled the legs WHILE the GTC TP ($50.02) was partially filling at the open (10 @ $54.56 — better price, as limits do); the OCO re-arm then used the ROW's stale qty (14) vs the broker's 4 → refused → remainder NAKED; at 9:40 the monitor closed the whole modeled row as closed_target (+$92.54 ×14) though the broker had sold only 10. HEALED live: protective STOP placed at the broker for the 4 shares (52.66 ≈ +10.2% locked; OCO was refused by Alpaca "must be exit orders" — quirk noted), closed row corrected to truth (10 @ 54.56 = +$67.70), new OPEN row tracks the 4 (stop 52.66 / target 56.40). CODE: leg_sync now fetches BROKER qty (min(row, live); returns "closed mid-dance" when 0) and, when an OCO is refused, places a PLAIN STOP (new brokers.submit_stop_sell) before giving up — `legs_naked_alert` fires only if even that fails. Net day so far: +$67.70 banked + 4 guarded runners.
> **2026-07-15 #3 (Nova) — Mike's correction applied: the step-out remainder reaches 65% PLUS (no ceiling); the 5/8 floor RATCHETS and is the exit.**
> Rewrote the multi-contract short branch: step 1 = half at 50% earned; each next step needs ~10pts more (TREZO_OPT_STEP_GAP); NO hard close at 65% — after any step the remainder rides with the volatility floor armed (buy back ALL when premium re-inflates 5–8% off its tracked low), exhaustion take at ~92% (TREZO_OPT_EXHAUST ratio 0.08), DTE rule unchanged. A single contract that came FROM a step-out (2→1) inherits the floor immediately (`_rid in _short_step`); a never-stepped single keeps 60% + floor-arm-at-55%. Shared 5/8 buffer computed once (dedup). "Catch the drawdown before completely selling out of the last of the option(s)."
> **2026-07-15 #2 (Nova) — short-premium harvest per Mike's morning spec: 50% STEPS for multi-contract, volatility-sized PROFIT FLOOR for singles.**
> Re-score short branch rewritten: multi-contract (≥2) buys back HALF at 50% of max profit (TREZO_OPT_STEP_AT) and the REST at 65% (TREZO_OPT_STEP2_AT ratio 0.35); single contracts keep the 60% rule but once ~55% is earned (TREZO_OPT_FLOOR_ARM ratio 0.45) a profit floor arms: `_short_low` tracks the premium's low, and a re-inflation of 5–8% off that low (buffer scaled by the underlying's realized daily vol: ~0.8%/day→5%, ~2.5%/day→8%) buys the leg back immediately ("a drawdown can be quick and hit the stop and shoot back up — lower stop based on volatility"). DTE≤5 @35% rule unchanged. Partial bookkeeping generalized to shorts (slice realized = credit − buy-back; signs correct on shrink/slice rows; direction income). Knowledge note doctrine updated. F 12.5P (52% earned, single) will arm its floor on the next re-score after restart.
> **2026-07-15 (Nova) — the INVISIBLE F-PUT: options reconcile made two-directional + evidence-based (Mike: "on Alpaca I see a put option but on Trezo there is no representation on card 3").**
> Design answer: wheel CSPs display under card 5 (Wheel), not card 3 (Options) — but BOTH were empty because the F 12.5P book row was closed on 7/6 by a reconcile FAIL-OPEN (broker options fetch failed → read as "no options at broker" → row closed "not present at broker") and the reconcile is one-directional, so after Alpaca restored the position on 7/8 nothing ever re-adopted it: NINE DAYS invisible while the broker held it (+52% of premium earned by 7/15 pre-market). FIXES in `_reconcile_with_broker`: (1) fetch error → SKIP user (never treat as empty); (2) HOLD guard — broker list empty + no closing fill found → keep the row open + `reconcile_hold` activity ("not closing on absence of evidence"); (3) ADOPT pass — any broker option with no open book row (matched by OCC across ALL open option rows, engine user always swept via TREZO_PRIMARY_USER_ID) is INSERTED as a tracked row (OCC parsed → strike/exp/type; short put→wheel_csp, short call→wheel_cc, long→long_*; credit/debit signed; `option_adopted` activity). (4) Open-rows query unfiltered (was wheel-only) so matching sees every lane. DB HEALED live: F 12.5P row reopened (status open, note explains the 7/6 fail-open) — Wheel card correct on refresh. PYPL pre-market +16.8%: its GTC TP limit ($50.02) fills AT THE OPEN at the better market price (~$55) — bracket legs are the overnight guard; peak-tracking catches up when the session tape starts. Restart loads the reconcile fixes.
> **2026-07-14 #10 (Nova) — the lab TEACHES the agents + capital story + SURGE pockets (+50%) + news sentiment in the scan (Mike: "how do I upload this for the agents' memory... 24% average... variability increased to 50 percent... add news sentiment").**
> (1) TEACH: truth first — every lab run ALREADY persists to backtest_runs (both web routes insert) and the per-stock selector reads that history (drops net-loss strategies, breaks ties). Added the narrative half: POST /lab/teach (agents) + /api/lab/teach proxy + "Teach the agents this run" button on merged results → top-20 findings become compact Mem0 notes ("lab[Merged — 5 lists]: AMSC best=orb ret +109.8% win 49% (37 trades)") + one `lab_teach` activity line; button confirms "Taught — N memory notes written". (2) CAPITAL STORY tile: summary grid gains "$10,000 → $12,400" (starting capital × equal-weight avg return; grid 4→5 cols). (3) SURGE DAYS: compass generals now carry volume pace (`vp`); new market_universe.surge_day() = median generals vp ≥1.4× AND mean |d1| ≥1.0%; _allocation_gate stretches the fast lanes (scalp/orb/stms/crypto/forex) to TREZO_SURGE_OVERFLOW_PCT (+50%, vs the normal +25%) on those days, `pocket_surge` activity once/day — "catch the wave, sell by the rules, recycle." (4) NEWS LENS: /lab/scan top-12 rows get 2-day Finnhub keyword sentiment (news_n/news_score/news_sent via data/news.assess) + a News column in MarketScanPanel. tsc/eslint/py_compile clean. Restart loads the agents half.
> **2026-07-14 #9 (Nova) — STRATEGY LAB rebuilt around Mike's workflow (portfolio charts, market scan → custom watchlists, merged/all-list backtests).**
> (1) LIVE PATTERNS = THE PORTFOLIO: strategy-lab page now feeds PatternsBoard the open paper_positions + options underlyings (forex excluded; falls back to the watchlist when flat) — "the live pattern charts should be what is being held." (2) MARKET/INDUSTRY SCAN: new agents GET /lab/scan (whole market = 3 generals/sector + most-actives; or one industry = ETF + its 10 generals; d1/d3/volume-pace per name, 10-min cache) + /api/lab/scan proxy + MarketScanPanel on the Simulation tab (sector dropdown, ranked table, tick names → SAVE AS CUSTOM WATCHLIST via new POST /api/watchlists/create using lib createWatchlist/addItem; router.refresh puts it straight into the sim picker + backtest merge). (3) BACKTEST MERGE: BacktestRunner single-select replaced with checkbox multi-select + "All watchlists at once" — merged + deduped universe, options skipped, run name "Merged — N lists"; compareAll (already present) races every strategy across the merged set. (4) OLD-SCALE SWEEP in the lab: /backtest + /backtest/compare defaults 700→70, compare route "700"→"70", runner tcs 650→65, sim lab 650→65, page copy rewritten to TCS 0–100 (30/25/20/15/10 components). tsc+eslint clean. Restart needed only for /lab/scan; the rest is web-side.
> **2026-07-14 #8 (Nova) — LEG-RESYNC: engine risk adjustments now reach Alpaca's REAL orders (Mike caught RBLX live: far $59.81 TP sat at the broker while price reversed).**
> Root cause: reevaluator `tighten_stop`/`lower_target` and the stock profit-trail ratchet persisted new prices to the DB only — broker-held bracket legs never moved. NEW `paper/leg_sync.resync_alpaca_legs(row, ...)`: verified cancel-legs-first dance (cancel_open_orders_for → poll drain ≤4s → submit_oco_sell at the row's current stop/target), long alpaca stock rows v1, `legs_resynced` activity, fail-open. Wired at all three adjustment sites. PLUS: `lower_target` now triggers on REVERSAL (giveback ≥ TIGHTEN_GIVEBACK 30% of peak) in addition to stale-3d — Mike: "cancel the open sell and re-enter it at a closer price." For RBLX that means: target 59.81 → ~56.24 (max(entry+0.5%, price+2%)) at the next hourly reeval pass, re-armed at the broker. ETH read live for Mike: stop already ratcheted to BREAKEVEN by the swing ladder (rungs +5%→lock 0 / +8%→+3% / +10%→+5%), target +12%, peak $1,877.74 ≈ 50% of the run (60% bank-step at ~$1,899.65 never hit) — trade can no longer go red. Restart loads.
> **2026-07-14 #7 (Nova) — PDT rule is GONE (verified) + flexible same-day capital + probability-based contract counts.**
> **VERIFIED via FINRA/SEC:** the pattern-day-trader designation AND the $25k minimum were ELIMINATED effective 2026-06-04 (SEC approved 2026-04-14, Reg Notice 26-10); only the standard $2k margin floor remains; brokers may phase in until 2027-10-20. Rewrote the gate accordingly: same-day lane now gates on TREZO_PDT_MIN_EQUITY ($2,000) instead of $25k/daytrade_count, and LEARNS from the broker — a PDT-flavored order reject during Alpaca's phase-in adds a `pdtstop:date` marker that stands the lane down for the day (`option_day_skip` activity). (2) FLEXIBLE SAME-DAY CAPITAL (Mike: "same day options and forex and crypto should have flexible capital"): day-options budget min($300 TREZO_DAY_OPT_USD, 8% eq TREZO_DAY_OPT_PCT) — was min($100, 2.5%); crypto + forex market types now get the same +25% intraday OVERFLOW the stock scalp lanes get in _allocation_gate. (3) CONTRACTS BY PROFIT PROBABILITY (Mike: "not cheap or expensive but the profit probability recovered in the shortest time"): arbitrary 2-contract cap removed — budget is the cap (ceiling TREZO_DAY_OPT_CT_MAX=10); dust premium <max($0.10, 0.4% of spot) skipped (lottery-grade, low odds of a FAST +30%); ITM-lean strike keeps delta responsive; directional lane ceiling 3→10 (budget-capped). Knowledge note + docx PDT text rewritten.
