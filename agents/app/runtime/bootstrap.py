...
def bootstrap_agents() -> None:
    ...
    registry.register(exit_adv,  "Exit Advisor (Phase 13d). Every 5 min watches every open position for the held-too-long pattern - tracks the running peak unrealized P&L and raises a dashboard alert when the position gives back a tiered share of its peak gain. Alert-only by default; when a book's Bot Tuning auto_exit_advisor is ON, an urgent giveback CLOSES the position on that book's own broker account (bound + route-checked, TE-18) and a warn trims half on internal paper rows.", role="observer")
    ...
    registry.register(bookhealth, "Book integrity monitor + TRADE QA INSPECTOR (2026-08-18; QA added 2026-09-02). Every 5 min asks one question per book: does it add up? Alarms on UNMANAGED NOTIONAL (broker holds it, the ledger has no row -- nothing is stopping it), positions sitting past their own stop, and a halt whose condition has already cleared. It also runs the trade QA pass (app/paper/trade_qa.py): a quality-control loop that checks every trade against the BROKER'S OWN PAPERWORK -- the order log and the fill activities, not a position snapshot -- and either passes it, or sends it back quarantined and named when the paperwork does not settle the question. It refreshes the working-order SHIELD every tick (one status=open read per book) so none of the FOUR paths that close a row on the broker's silence -- the stock reconcile, the option reconcile, and position_monitor's stock/option and crypto lanes -- closes one while an order for it is still in flight; each of them skips the close both when the answer is 'yes, still working' and when it is 'could not check', and it repairs NOTHING unless TREZO_QA_AUTOFIX is turned on (it ships OFF: every repair is logged as qa_would_fix instead). It never places, cancels or modifies a broker order, and it never writes a stop or a target. Sends OUT through the alert webhook rather than into a table nobody reads.", role="observer")
